...
def generate_svg():
    ...
    for i in range(0, num_lines, 4):
        ...
        for j in range(3, num_circles, 2):
            ...
            dx = cx + dot_r * math.cos(angle)
            dy = cy + dot_r * math.sin(angle)
            elements.append(f'<circle cx="{dx:.1f}" cy="{dy:.1f}" r="3.0" fill="#8bffdb" fill-opacity="0.6"/>')
    for i in range(0, num_lines, 2):
    ...
    for i in range(0, num_lines, 4):
        ...
        for j in range(2, num_circles, 3):
            ...
            dx = cx + dot_r * math.cos(angle)
            dy = cy + dot_r * math.sin(angle)
            elements.append(f'<circle cx="{dx:.1f}" cy="{dy:.1f}" r="3.0" fill="#86f2c7" fill-opacity="0.6"/>')
    ...
    elements.append(f'<circle cx="{cx}" cy="{cy}" r="8.0" fill="#ddb5d5" fill-opacity="0.9"/>')
    elements.append('</svg>')
    return '\n'.join(elements)
