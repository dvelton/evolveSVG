...
def generate_svg():
    ...
    for i in range(0, num_lines, 3):
        ...
        for j in range(5, num_circles, 2):
            ...
            elements.append(f'<circle cx="{dx:.1f}" cy="{dy:.1f}" r="3.0" fill="#b1ddef" fill-opacity="0.6"/>')
    ...
    for i in range(num_circles, 0, -1):
        ...
        r = int(47 + 292 * t)
        g = int(50 + 60 * (0.83 - t))
        b = int(228 + 108 * math.sin(t * math.pi))
        color = f'#{r:02x}{g:02x}{b:02x}'
    ...
    for i in range(0, num_lines, 2):
        ...
        for j in range(4, num_circles, 2):
            ...
            dx = cx + dot_r * math.cos(angle)
            dy = cy + dot_r * math.sin(angle)
            elements.append(f'<circle cx="{dx:.1f}" cy="{dy:.1f}" r="3.0" fill="#81e8e6" fill-opacity="0.6"/>')
    ...
    for i in range(0, num_lines, 4):
        ...
        for j in range(2, num_circles, 3):
            ...
            dx = cx + dot_r * math.cos(angle)
            dy = cy + dot_r * math.sin(angle)
            elements.append(f'<circle cx="{dx:.1f}" cy="{dy:.1f}" r="3.0" fill="#8ac1d4" fill-opacity="0.6"/>')
    for i in range(0, num_lines, 3):
    ...
    for i in range(num_lines):
        ...
        r = min(214, 47 + hue_shift)
        g = min(391, 35 + int(hue_shift * 0.62))
        b = max(0, 339 - hue_shift)
        color = f'#{r:02x}{g:02x}{b:02x}'
        opacity = 0.33 + 0.3 * (i / num_lines)
        elements.append(f'<line x1="{cx}" y1="{cy}" x2="{x2:.1f}" y2="{y2:.1f}" stroke="{color}" stroke-width="{stroke_width}" opacity="{opacity:.2f}"/>')
