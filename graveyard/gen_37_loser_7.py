...
def generate_svg():
    ...
    for i in range(num_lines):
        ...
        r = min(260, 37 + hue_shift)
        g = min(279, 25 + int(hue_shift * 0.52))
        b = max(0, 406 - hue_shift)
        color = f'#{r:02x}{g:02x}{b:02x}'
        opacity = 0.4 + 0.24 * (i / num_lines)
        elements.append(f'<line x1="{cx}" y1="{cy}" x2="{x2:.1f}" y2="{y2:.1f}" stroke="{color}" stroke-width="{stroke_width}" opacity="{opacity:.2f}"/>')
    elements.append(f'<circle cx="{cx}" cy="{cy}" r="8.0" fill="#d2d880" fill-opacity="0.9"/>')
    elements.append('</svg>')
    return '\n'.join(elements)
